...
/// # Errors
///
/// This function will print an error message and return `None` if `item` is not ASCII alphabetic.
...
/// Takes inventory of the items present in a sequence, storing whether or not an item is present
/// in the bit corresponding to the item's priority of a `u64`
///
/// # Arguments
///
...
fn items_present_in_sequence(item_sequence: &str) -> u64 {
    ...
    for item in item_sequence.chars() {
        if let Some(priority) = try_item_to_priority(item) {
...
/// https://adventofcode.com/2022/day/3
///
/// This uses `items_present_in_sequence` to take inventory of the first compartment, then returns
/// early when a matching item is found in the second compartment.
///
/// # Arguments
///
/// * `rucksack` - A string slice representing the items present in a rucksack, with the first half
///   of the characters representing the first compartment, and vice versa with the second half and
///   second compartment
///
/// # Errors
///
/// If the string slice does not have an even length, an error message is printed, and the last byte
/// is ignored
///
/// If the byte at the midpoint of the string slice is not a `char` boundary, `rucksack` is
/// ill-formatted for this problem. In this case, an error message is printed, and 0 is returned.
///
/// If an item couldn't be found that's present in both compartments, an error message is printed,
/// and 0 is returned.
fn compute_priority_of_rucksack(rucksack: &str) -> u32 {
    ...
    let first_compartment_items: u64 = items_present_in_sequence(&rucksack[..midpoint]);

    for second_compartment_item in rucksack[midpoint..].chars() {
        if let Some(priority) = try_item_to_priority(second_compartment_item) {
...
///   `'\n'`
fn sum_rucksack_priorities(rucksacks: &str) -> u32 {
    rucksacks
        .split('\n')
        .map(compute_priority_of_rucksack)
        .sum()
}
...
/// used.
fn sum_rucksack_priorities_for_groups(rucksacks: &str, mut group_size: usize) -> u32 {
    ...
    for (rucksack_index, rucksack_items) in rucksacks
        .split('\n')
        .map(items_present_in_sequence)
        .enumerate()
    {
...
fn sum_rucksack_priorities_for_groups_of_3(rucksacks: &str) -> u32 {
    sum_rucksack_priorities_for_groups(rucksacks, 3_usize)
}

fn main() {
    let args: Args = Args::parse();
    let input_file_path: &str = args.input_file_path("input/day3.txt");
        ...
        unsafe {
            open_utf8_file(input_file_path, |input: &str| {
                println!(
                    ...
                    sum_rucksack_priorities(input),
                    sum_rucksack_priorities_for_groups_of_3(input)
                );
            })
        }
    {
